...
# years = ['2023\nKigali', '2024\nVienna', '2025\nSingapore']
# emissions_data = {
#     'Air Travel': np.array([5073.5, 5480.7, 10157.7]),
#     'Accommodation': np.array([197.2, 716.1, 3496.7]),
#     'Meals': np.array([92.1, 150.7, 245.1]),
#     'Venue': np.array([53.6, 87.6, 142.5])
# }
# categories = list(emissions_data.keys())
# data = np.array(list(emissions_data.values()))

# # --- Calculations ---
# # Calculate total emissions for each year to find percentages
...
# # --- Plotting ---
# fig, ax = plt.subplots(figsize=(12, 8)) # Increased figure size for better label spacing

# # Create the stacked bars
# bottom = np.zeros(len(years))
# for i, category in enumerate(categories):
#     bar = ax.bar(years, data[i], bottom=bottom, label=category, color=colors[i])
#     bottom += data[i]
    
#     # Add data labels with percentages for significant segments
#     for j, rect in enumerate(bar):
#         height = rect.get_height()
#         # Only label segments that are large enough to read clearly
#         if height > 500: 
#             percentage = (height / totals[j]) * 100
#             # Format label with value and percentage on a new line
...
#     ax.text(j, total + 150, # Position label slightly above the bar
#             f'Total:\n{total:,.0f}', 
#             ha='center', 
#             va='bottom', 
#             fontsize=10, 
...
# # --- Titles and Labels ---
# ax.set_title('ICLR Conference Carbon Emissions (2023-2025)', fontsize=16, fontweight='bold', pad=20)
...
# ax.legend(title='Emission Source', bbox_to_anchor=(1.02, 1), loc='upper left')

# # Improve layout and formatting
# ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: format(int(x), ',')))
# ax.set_ylim(top=max(totals) * 1.15) # Increase y-axis limit to make space for total labels
# plt.xticks(fontsize=11)
# plt.yticks(fontsize=11)
# plt.tight_layout(rect=[0, 0, 0.85, 1])

# # --- Save and Show ---
# plt.savefig('ICLR_Emissions_Graph_Enhanced.png', dpi=300, bbox_inches='tight')
# plt.show()
